...
import com.techtest.todoapp.data.source.network.toLocal
import com.techtest.todoapp.di.ApplicationScope
import com.techtest.todoapp.di.DefaultDispatcher
...
class DefaultTaskRepository @Inject constructor(
    ...
    override suspend fun refresh() {
        withContext(dispatcher) {
            ...
            localDataSource.deleteAll()
            val list = ArrayList<LocalTask>()
            for (task in remoteTasks) {
                list.add(task.toLocal())
            }
            localDataSource.upsertAll(list)
        }
    }
